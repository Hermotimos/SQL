...
CORRECT ANSWER: c >= (SELECT SUM(b) FROM R)
*/
...
CORRECT ANSWER: Add "UNIQUE" after each of the first two INT's, and add "REFERENCES Emps(id,ssNo)" before the closing parenthesis.
EXPLANATION: It's not possible to create composite FK for single column. How would single managerIT be identical with composite key (id, ssNo)?
*/
...
CORRECT ANSWER: Inserting (7,5) into T
*/
...
CORRECT ANSWER: Updating (1,5) in T to be (1,2)
*/
...
CORRECT ANSWER: delete from R where f>3
*/
...
CORRECT ANSWER: update R set e=e+1 where f>=3
*/